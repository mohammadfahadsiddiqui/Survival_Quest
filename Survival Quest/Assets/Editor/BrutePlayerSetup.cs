...
using SurvivalGame;
using SurvivalQuest.Environment;
...
public static class BrutePlayerSetup
{
    ...
    public static void SetupBrutePlayer()
    {
        ...
        EnsureSampleSceneEnvironment();
    }

    private static void EnsureSampleSceneEnvironment()
    {
        ...
        GameObject root = GameObject.Find("EnvironmentGenerator");
        if (root == null)
        {
            root = new GameObject("EnvironmentGenerator");
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(scene);
        }

        var bootstrap = root.GetComponent<EnvironmentAutoBootstrap>();
        if (bootstrap == null)
        {
            root.AddComponent<EnvironmentAutoBootstrap>();
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(scene);
        }

        var generator = root.GetComponent<ProceduralEnvironmentGenerator>();
        if (generator == null)
        {
            generator = root.AddComponent<ProceduralEnvironmentGenerator>();
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(scene);
        }
